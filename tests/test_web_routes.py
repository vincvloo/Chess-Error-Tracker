...
from chess_tracker.web.jobs import (BIG_UPDATE_THRESHOLD, FIRST_RUN_GAME_LIMIT,
                                    FIRST_RUN_PARALLEL_THRESHOLD, JobAlreadyRunningError)
...
def test_onboarding_new_username_starts_capped_job_and_redirects_to_demo_dashboard(
    ...
    # forces the capped first batch across workers instead of running it
    # serially, which below analysis_runner's normal 200-game default it
    # otherwise wouldn't -- see jobs.FIRST_RUN_PARALLEL_THRESHOLD.
    assert calls["kwargs"]["parallel_threshold"] == FIRST_RUN_PARALLEL_THRESHOLD
